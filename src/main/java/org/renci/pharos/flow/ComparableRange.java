...
public class ComparableRange implements Comparable<ComparableRange> {
	...
	public ComparableRange(String s)throws FlowPharosException{
		s = s.trim();
		String[] ss = null;
		...
		if (ss.length==1){
			try {
				...
				throw new FlowPharosException("accept decimal number");
			}
		}
		else if(ss.length==2){
			try {
				...
				if (a>b)throw new FlowPharosException("accepted format: a-b, where a<=b");
				range=Range.closed(a,b);
			}
			catch (Exception ex){
				throw new FlowPharosException("accept decimal number-decimal number");
			}
		}
		else {
			throw new FlowPharosException("invalid format, shoudl be decimal number-decimal number");
		}
	}
