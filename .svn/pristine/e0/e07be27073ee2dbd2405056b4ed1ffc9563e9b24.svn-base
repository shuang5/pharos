...
public class FlowUnit implements Comparable<FlowUnit>, Iterable<BigInteger>{
	...
	public FlowUnit(String s) throws FlowPharosException{
		String delims="[ ]*/[ ]*";
		//System.out.println("Flow Unit init String is "+s);
		String[] subTokens=s.split(delims);
		if (subTokens.length !=2) 
			throw new FlowPharosException("wrong flowspace format -- decimal number-decimal number/decimal number-decimal number");
		...
		if(b>1<<(stop-start+1))
			throw new FlowPharosException("wrong format -- out of range specified by the mask");
	...
	public FlowUnit(ComparableRange[] valnMsk) throws FlowPharosException
	{
		int VALUE = 0;
		int MASK = 1;
