...
public class Rules {
	...
	public void deleteLastRuleTable() throws FlowPharosException{
		if(ruletables.size() > 0)
			ruletables.remove(ruletables.size()-1);
		else {
			throw new FlowPharosException("Rule table is empty");
		}
	}
	public JSONArray getRuleTable(int i) throws FlowPharosException{
		try {
			return ruletables.getJSONArray(i);
		}
		catch (JSONException je){
			throw new FlowPharosException ("index out of boundary");
		}
	...
	*/
	public JSONArray addRuleToRuleTable(Rule r, int i) throws FlowPharosException{
		JSONArray rt=getRuleTable(i);
		rt.add(r.getJSONObject());
		return ruletables.element(i,rt);
	}
	public JSONArray addRule(Rule r) throws FlowPharosException{
		JSONArray rt=new JSONArray();
		if(ruletables.size()>0)
	...
	private JSONArray setRule(Rule r, int i) throws FlowPharosException{
		JSONArray rt=new JSONArray();
		if(ruletables.size()> 0)
			rt=getRuleTable(ruletables.size()-1);
		else
	...
	public JSONArray deleteRule(int i) throws FlowPharosException{
	...
	public FlowSpace evaluate (FlowSpace fs) throws FlowPharosException{
		...
		while(rt_it.hasNext()){
			...
			while(r_it.hasNext()){
				try {
					...
					throw new FlowPharosException("wrong rule format");
				}
			}
		}
		return output;
