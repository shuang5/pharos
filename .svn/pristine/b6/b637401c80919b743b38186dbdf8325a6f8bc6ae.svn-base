...
class ProviderGraphReader implements PharosGraphReader{
	...
	public FlowVariables4D add2(Graph graph) {
	...
	public FlowVariables4D add(Graph graph) {
		...
		for (Vertex v: it){
		    Iterable<Edge> ite=v.getEdges(Direction.OUT);
		    for (Edge e: ite){
		  	    Map<FlowUnit, FlowUnit> labels= new HashMap<FlowUnit,FlowUnit>();
		  	    //get labels from graph
		  	    String peers=(String)e.getProperty("Peers");
		  	    String[] tokens = peers.split("[ (,)]+");
		  	    String[] port1=tokens[1].split("[:]");
					...
					if(s.equals("Yes")){
						if(e.getProperty("Rules")==null){
							...
							while(fit.hasNext()){
								...
								FlowLabels llin=new FlowLabels(fsin);
								FlowLabels llout=new FlowLabels(rules.evaluate(fsin));
								Iterator<FlowUnit> itllin=llin.fiterator();
								Iterator<FlowUnit> itllout=llout.fiterator();
								while (itllin.hasNext() && itllout.hasNext()){
									labels.put(itllout.next(), itllin.next());
								}
							}
						}
					}
						...
						while (itin.hasNext() && itout.hasNext()){
							labels.put(itout.next(), itin.next());
						}
					}
	...
	public static void main(String[] args){
		...
		try{
		GraphMLReader.inputGraph(graph, new FileInputStream("/Users/shuang/Desktop/Pharos DOE/4nodes"));
		}
		catch(Exception e){
			e.printStackTrace();
		}
